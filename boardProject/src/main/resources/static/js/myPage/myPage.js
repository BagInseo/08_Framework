...
//img태그 (프로필 이미지가 보여지는 요소)
const profileImg = document.querySelector("#profileImg");

//input type="file" 태그 (실제 업로드할 프로필 미니지를 선택하는 요소 )
...
const changeImageFn = e => {
  ...
  //업로드된 파일이 없으면 files[0] == undifined
  ...
  //---------업로드된 파일이 없다면(취소한 경우)----------
  if(file == undefined){
    console.log("파일 선책 후 취소됨");
    
    //파일 선택 후 취소 -> value == ''
    // -> 선택한 파일이 없음으로 기록됨
    // -> backupInput으로 교체 시켜서 
    //    이전 이미지가 남아있는 것 처럼 보이게 함
    ...
    //imageInput 변수에 백업을 대입하여 대신하도록 함
    imageInput=backupInput;

    //회면에 추가된 백업본에는 이벤트 리스너가 
    // 존재하지 않기 때문에 추가
    imageInput.addEventListener("change",changeImageFn);

    //한번 화면에 추가된 요소(backupInput)는 재사용이 불가능
...
//change이벤트 : 새로운 값이 기존 값과 다를 경우 발생
imageInput.addEventListener("change",changeImageFn);
