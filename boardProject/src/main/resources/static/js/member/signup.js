...
/* 필수 입력 항목의 유효성 검사 여부를 체크하기 위한 객체(체크리스트)
    - ture == 해당 항목은 유효한 형식으로 작성됨
    - false == 해당 항목은 유효하지 않은 형식을 작성됨
*/
const checkObj = {
    "memberEmail" : false,
    "memberPw" : false,
...
memberPw.addEventListener("input", e=>{
    ...
    //4) 입력 받은 비밀번호 정규식 검사
    const regExp=/^(?=.*[A-Za-z0-9!@#_-])[A-Za-z0-9!@#_-]{6,20}$/;
...
memberTel.addEventListener("input",e=>{
    ...
    if(!regExp.test(inputTel)){
        telMessage.innerText="유효하지 않은 전화번호 입니다.";
...
//-------------------------------------------------------
/* 이메일 인증 */
const sendAuthKeyBtn= document.querySelector("#sendAuthKeyBtn"); //인증 번호 받기 버튼btn
const authKey= document.querySelector("#authKey"); //인증번호 입력input
const checkAuthKeyBtn= document.querySelector("#checkAuthKeyBtn"); //인증번호 확인 버튼btn
const authKeyMessage= document.querySelector("#authKeyMessage"); //인증번호 관련 메시지 출력span
