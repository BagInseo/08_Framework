...
public class EditBoardController {
	...
	public String boardInsert(
		...
		if(boardNo>0) {
			path="/board/" + boardCode + "/" + boardNo; //상세조회
	...
	/*
	 * @PostMapping("{boardCode:[0-9]+}/{boardNo:[0-9]+}/delete") public String
	 * deleteBoard(
	 * 
	 * @PathVariable("boardCode") int boardCode,
	 * 
	 ...
	 * String path=null;
	 * 
	 * if(result>0) { path="/board/"+ boardCode; } else {
...
//	@PostMapping("{boardCode:[0-9]+}/{boardNo:[0-9]+}/delete")
			...
			@RequestParam(value="querystring", required = false, defaultValue = "") String querystring ) throws IllegalStateException, IOException 
	{
		...
		// 1. 커멘ㄷ 객체(inputBoard)ㅇ[ boardCode,boardNo, memberNo세팅
		inputBoard.setBoardCode(boardCode);
