...
const inputImageList=document.getElementsByClassName("inputImage");  //input태그 5개
...
/* ***** input 태그 값 변경 시(파일 변경 시) 실행할 함수 ***** */
/**
 * 
...
const changeImageFn=(inputImage,order)=>{
    ...
    if(file==undefined){
        ...
        /* 한 번 화면에 추가된 요소는 재사용이 불가능 */
        
        // 백업본을 한 번 더 복제
        const temp= backupInputList[order].cloneNode(true);

        inputImage.after(temp); /* 백업본은 다음 요소로 추가 */
    ...
    //------------선택한 파일의 크기가 최대 크기(maxSize) 초과 -----------

    if(file.size>maxSize){
        alert("10MB 이하의 이미지를 선택해주세요");

        //해당 순서에 backup요소가 없거나
        ...
        //이전에 정상 선택 -> 다음 선택에서 이미지 크기를 초과한 경우
    ...
    reader.addEventListener("load", e=>{
        ...
        //img태그(.preview)에 src 속성으로 url 값을 대입
...
for(let i = 0; i<inputImageList.length ; i++){
    ...
    deleteImageList[i].addEventListener("click", ()=>{
        ...
        //img, input, backup의 인덱스가 모두 일치하다는 특징을 이용
