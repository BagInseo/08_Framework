...
//------------------------------------------------------
/* 특정 회원 탈퇴 복구 */
const restorationMemberNo = document.querySelector("#restorationMemberNo"); //복구할 회원 번호
const restorationMemberNoBtn = document.querySelector("#restorationMemberNoBtn"); //탈퇴 복구 버튼


restorationMemberNoBtn.addEventListener("click",()=>{
    ...
    .then(resp=>resp.text())
    .then(result=>{
        if(result>0) alert("회원 탈퇴 복구 완료");
        else         alert("회원 탈퇴 복구 실패");
    });
